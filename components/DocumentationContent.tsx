      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <aside className="w-full lg:w-72 flex-shrink-0">
          <div className="bg-white border border-slate-200 rounded-[2rem] overflow-hidden shadow-sm">
            <div className="px-8 py-6 bg-blue-900 text-white text-xs font-black uppercase tracking-widest flex items-center">
              <FileText size={18} className="mr-3 text-red-500" /> Documentation
            </div>
            <button className="w-full px-8 py-6 text-slate-400 text-xs font-black uppercase tracking-widest border-b border-slate-100 bg-slate-50 flex items-center hover:text-red-600 transition-colors">
        ...
        <div className="flex-1 space-y-8">
          <div className="bg-white p-12 rounded-[3rem] shadow-sm border border-slate-100 border-t-8 border-red-600">
            ...
            <p className="text-slate-500 text-base leading-relaxed mb-8 font-medium">
              To ensure 24-hour tag activation, please keep the following scanned documents ready. Our automated system verifies documents instantly.
            </p>
            
            <ul className="space-y-6">
              {documents.map((doc, idx) => (
                <li key={idx} className="flex items-start">
                  <div className={`mt-1 mr-4 ${idx % 2 === 0 ? 'text-red-500' : 'text-blue-500'}`}>
                    <CheckCircle2 size={24} className="fill-current opacity-10" />
                  </div>
          ...
          <div className="bg-red-50 rounded-[2rem] p-8 border border-red-100">
            <h4 className="text-red-900 font-black uppercase text-[10px] tracking-[0.2em] mb-4">Pro Tip</h4>
